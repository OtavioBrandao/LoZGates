...
Coleta dados anonimizados granulares sobre o uso da aplicação para análise detalhada.
"""
...
class DetailedUserLogger:
    """Sistema de logging detalhado para coleta de dados granulares de uso."""
    ...
    def __init__(self, app_version="1.0-beta"):
        ...
        self.current_session = {
            ...
            "interactive_circuit": {
                "sessions_started": 0,
                "components_added": {},  # tipo -> quantidade
            ...
            "equivalence_analysis": {
                ...
                "expression_pairs": []  # Lista de pares analisados
            },
            
            "expression_patterns": {
                "variable_counts": {},  # quantidade_vars -> frequência
                "operator_usage": {"AND": 0, "OR": 0, "NOT": 0},
                "expression_lengths": {},  # tamanho -> frequência
                "common_expressions": {}  # hash -> frequência (para privacidade)
            }
        }
    ...
    def _generate_anonymous_id(self) -> str:
        """Gera ID anônimo baseado no hardware do usuário."""
        try:
    ...
    def _load_settings(self):
        """Carrega configurações de logging de forma segura."""
        try:
    ...
    def _initialize_log_file(self):
        """Inicializa arquivo de log se não existir ou estiver corrompido."""
        try:
    ...
    def _hash_expression(self, expression: str) -> str:
        """Cria hash da expressão para análise de padrões sem expor conteúdo."""
        return hashlib.md5(expression.encode()).hexdigest()[:8]
    
    def log_event(self, event_type: str, event_data: Dict[str, Any] = None):
        """Registra um evento no log com timestamp detalhado."""
    ...
    def log_expression_entered(self, expression: str, is_valid: bool):
        """Registra entrada de uma expressão com análise detalhada."""
        if not self.logging_enabled:
            return
        ...
        # Conta operadores
        patterns["operator_usage"]["AND"] += expression.count("*")
        patterns["operator_usage"]["OR"] += expression.count("+")
        patterns["operator_usage"]["NOT"] += expression.count("~")
    ...
    def log_interactive_simplification_start(self, expression: str):
        """Inicia uma sessão de simplificação interativa."""
        if not self.logging_enabled:
            return
    ...
    def log_law_applied(self, law_name: str, success: bool, step_number: int):
        """Registra aplicação de lei na simplificação interativa."""
        if not self.logging_enabled:
            return
    ...
    def log_simplification_skip(self, step_number: int):
        """Registra uso do botão 'pular' na simplificação."""
        if not self.logging_enabled:
            return
    ...
    def log_simplification_undo(self):
        """Registra uso do undo na simplificação."""
        if not self.logging_enabled:
            return
    ...
    def log_simplification_completed(self, total_steps: int, laws_used: List[str]):
        """Registra conclusão de uma simplificação."""
        if not self.logging_enabled:
            return
    ...
    def log_circuit_interaction_start(self):
        """Inicia uma sessão de circuito interativo."""
        if not self.logging_enabled:
            return
    ...
    def log_component_action(self, action: str, component_type: str = None):
        """Registra ações com componentes no circuito interativo."""
        if not self.logging_enabled:
            return
    ...
    def log_circuit_test(self, success: bool, attempt_number: int = None):
        """Registra teste de circuito."""
        if not self.logging_enabled:
            return
    ...
    def log_equivalence_check(self, expression1: str, expression2: str, result: bool):
        """Registra verificação de equivalência com detalhes."""
        if not self.logging_enabled:
            return
        ...
        # Adiciona par à lista (com hash para privacidade)
        pair_data = {
            "expr1_hash": self._hash_expression(expression1),
            "expr2_hash": self._hash_expression(expression2),
            "expr1_length": len(expression1.replace(" ", "")),
            "expr2_length": len(expression2.replace(" ", "")),
            "result": result,
            "timestamp": datetime.now().isoformat(),
        ...
        self.log_event("equivalence_check", {
            "result": result,
            "expr1_vars": len(set(c for c in expression1 if c.isalpha())),
            "expr2_vars": len(set(c for c in expression2 if c.isalpha())),
            "complexity_difference": abs(len(expression1) - len(expression2)),
            "check_sequence": equiv_data["total_checks"]
        })
    
    def log_feature_used(self, feature_name: str, duration_seconds: float = None):
    ...
    def log_error(self, error_type: str, error_message: str, context: str = None):
        """Registra erros encontrados pelo usuário."""
        if not self.logging_enabled:
            return
    ...
    def log_tab_changed(self, from_tab: str, to_tab: str):
        """Registra mudança de aba com timing."""
        if not self.logging_enabled:
            return
    ...
    def end_session(self):
        """Finaliza a sessão atual e calcula estatísticas finais."""
        if not self.logging_enabled:
            return
    ...
    def get_detailed_summary(self) -> Dict[str, Any]:
        """Retorna resumo detalhado do uso da aplicação."""
        try:
    ...
    def _aggregate_detailed_stats(self, sessions: List[Dict], summary: Dict):
        """Agrega estatísticas detalhadas de todas as sessões."""
    ...
    def should_prompt_data_sharing(self) -> bool:
        """Verifica se deve mostrar prompt para compartilhar dados."""
        return True  # Para testes, sempre mostra
    
    def create_shareable_data(self) -> Dict[str, Any]:
        """Cria dados anonimizados detalhados para compartilhamento."""
        try:
            return {
                "app_version": self.app_version,
            ...
            print(f"Erro ao criar dados compartilháveis: {e}")
            return {}


class DetailedDataSharingDialog:
    """Dialog aprimorado para mostrar dados detalhados."""
    ...
    def show_dialog(self) -> bool:
        """Mostra dialog com preview detalhado dos dados."""
...
• Personalizar a experiência de aprendizado"""
    ...
    def _create_data_preview(self, summary: Dict[str, Any]) -> str:
        """Cria uma visualização estruturada dos dados para o usuário."""
        lines = []
        ...
        lines.append(f"• Pares equivalentes encontrados: {equiv.get('equivalent_found', 0)}")
        lines.append(f"• Pares não equivalentes: {equiv.get('non_equivalent_found', 0)}")
        
        recent_checks = equiv.get('recent_checks', [])[:5]
        if recent_checks:
            lines.append("• Últimas verificações (apenas hashes):")
            for i, check in enumerate(recent_checks, 1):
                result = "✓" if check.get('result') else "✗"
                lines.append(f"  {i}. {result} ({check.get('expr1_length')}chars vs {check.get('expr2_length')}chars)")
        lines.append("")
        ...
        if var_counts:
            lines.append("• Quantidade de variáveis por expressão:")
            for count, freq in list(var_counts.items())[:5]:
                lines.append(f"  - {count} variáveis: {freq} expressões")
        
        operators = patterns.get('operator_preferences', {})
        if operators:
            lines.append("• Uso de operadores:")
            for op, count in operators.items():
                lines.append(f"  - {op}: {count} vezes")
        lines.append("")
...
class GoogleFormsSubmitter:
    """Classe para envio de dados detalhados via Google Forms."""
    ...
    def submit_data(self, data: Dict[str, Any]) -> bool:
        """Envia os dados detalhados para o Google Forms."""
        try:
            # Converte os dados detalhados para JSON
            detailed_json = json.dumps(data.get("detailed_summary", {}), indent=2, ensure_ascii=False)
            ...
            form_data = {
                ...
                self.entry_mapping['summary_json']: detailed_json
            }
            ...
            if response.status_code == 200:
                print("✅ Dados detalhados enviados com sucesso!")
                return True
            else:
            ...
            print(f"❌ Erro de conexão ao enviar dados: {e}")
            return False
        except Exception as e:
            print(f"❌ Erro inesperado durante o envio: {e}")
            return False